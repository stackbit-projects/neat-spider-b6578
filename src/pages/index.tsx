
import React from "react";
function Index() {
  return (
    <div style={{ width: "100%", padding: "100px", alignContent: "center" }}>
      <header>
        <img src="data:image/svg+xml;base64," alt="" />
        <h1 style={{ margin: 0 }}>
          Welcome to Plasmic!
        </h1>
        <h4>
          <a
            style={{ color: "blue" }}
            href="https://www.plasmic.app/learn/"
            target="_blank"
            rel="noopener noreferrer"
          >
            Learn Plasmic
          </a>
        </h4>
        
          <h3>Your pages:</h3>
          <ul>
            <li><a style={{ color: "blue" }} href="/landing">Landing - /landing</a></li>
<li><a style={{ color: "blue" }} href="/products">Products - /products</a></li>
<li><a style={{ color: "blue" }} href="/explore">Explore - /explore</a></li>
<li><a style={{ color: "blue" }} href="/contact">Contact - /contact</a></li>
<li><a style={{ color: "blue" }} href="/products-old-eng-tee">ProductsOldEngTee - /products-old-eng-tee</a></li>
<li><a style={{ color: "blue" }} href="/products-tees">ProductsTees - /products-tees</a></li>
<li><a style={{ color: "blue" }} href="/products-fuck-12-tee">ProductsFuck12Tee - /products-fuck-12-tee</a></li>
<li><a style={{ color: "blue" }} href="/products-slogan-tee">ProductsSloganTee - /products-slogan-tee</a></li>
<li><a style={{ color: "blue" }} href="/products-rubber-patch-tee">ProductsRubberPatchTee - /products-rubber-patch-tee</a></li>
<li><a style={{ color: "blue" }} href="/products-hoodies">ProductsHoodies - /products-hoodies</a></li>
<li><a style={{ color: "blue" }} href="/products-jackets">ProductsJackets - /products-jackets</a></li>
<li><a style={{ color: "blue" }} href="/products-filthy-states-of-amerikka-jacket">ProductsFilthyStatesOfAmerikkaJacket - /products-filthy-states-of-amerikka-jacket</a></li>
<li><a style={{ color: "blue" }} href="/products-security-jacket">ProductsSecurityJacket - /products-security-jacket</a></li>
<li><a style={{ color: "blue" }} href="/products-striped-hoodie">ProductsStripedHoodie - /products-striped-hoodie</a></li>
<li><a style={{ color: "blue" }} href="/products-striped-weapon-hoodie">ProductsStripedWeaponHoodie - /products-striped-weapon-hoodie</a></li>
<li><a style={{ color: "blue" }} href="/trial">Trial - /trial</a></li>
          </ul>
    
        <p><i>Note: Remember to remove this file if you introduce a Page component at the '/' path.</i></p>
      </header>
    </div>
  );
}

export default Index;
  